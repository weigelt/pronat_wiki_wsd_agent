...
import edu.kit.ipd.pronat.wiki_wsd.util.TestHelper;
import edu.kit.ipd.pronat.wiki_wsd.util.Text;
...
public class WordSenseDisambiguationTest {
	...
	public void testAccuracy() {
		...
		for (String id : textsStrings) {
			...
			String input = text.getText().replace("\n", " ");
			...
			if (logger.isInfoEnabled()) {
				...
				logger.info("Evaluation at " + df.format(currProgress) + " with an accuracy of currently " + currentAccuracy);
			}
		}
			...
			for (String[] wsdAnnotation : expectedAnnotations) {
				...
				if (!expectedWsd.equals(classifiedWsd) && !wsdAnnotation[3].equals(classifiedWsd)) {
					...
					if (classifiedWsd == null) {
						...
						System.out.println(ppd.getTaggedHypothesis(0).toString());
					}
				}
			}
			if (allCorrect) {
